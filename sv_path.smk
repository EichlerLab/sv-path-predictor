...
SKIPVCF = config.get('skipvcf', False)
manifest_df = pd.read_csv(MANIFEST, sep='\t', index_col='sample', keep_default_na=False)

def find_vcf(wildcards):
    return manifest_df.at[wildcards.sample, 'vcf_file']

def find_sample_bed6(wildcards):
    # bed6's headers: #CHROM\tPOS\tEND\tID\tSVTYPE\tSVLEN
    return manifest_df.at[wildcards.sample, 'bed6']
...
checkpoint split_by_type:
    input:
        bed = rules.convert_tab.output.bed if not SKIPVCF else find_sample_bed6
    output:
